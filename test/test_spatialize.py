...
def test_esi_idw_2d(op='estimate'):
    ...
    if op=='estimate':
        values = libspatialize.esi_idw_2d(np.float32(samples[['x', 'y']].values), np.float32(samples[['cu']].values[:,0]), 100, 0.7, 2.0, np.float32(locations[['X','Y']].values))
        ...
        values = libspatialize.loo_esi_idw_2d(np.float32(samples[['x', 'y']].values), np.float32(samples[['cu']].values[:,0]), 100, 0.7, 2.0, np.float32(locations[['X','Y']].values))
        ...
        values = libspatialize.kfold_esi_idw_2d(np.float32(samples[['x', 'y']].values), np.float32(samples[['cu']].values[:,0]), 100, 0.7, 2.0, k, np.float32(locations[['X','Y']].values))
        df = pd.DataFrame(samples, columns=['X', 'Y', 'cu'])
...
def test_esi_idw_3d(op='estimate'):
    ...
    if op=='estimate':
        values = libspatialize.esi_idw_3d(np.float32(samples[['X','Y','Z']].values), np.float32(samples[['cu']].values[:,0]), 100, 0.7, 2.0, np.float32(locations[['X','Y','Z']].values))
        df = pd.DataFrame(locations, columns=['X', 'Y', 'Z'])
        df['py_cu'] = np.nanmean(values, axis=1)
        df.to_csv('./test/testdata/output/pyesi_idw_3d.csv', index=False)
    elif op=='loo':
        values = libspatialize.loo_esi_idw_3d(np.float32(samples[['X','Y','Z']].values), np.float32(samples[['cu']].values[:,0]), 100, 0.7, 2.0, np.float32(locations[['X','Y','Z']].values))
        df = pd.DataFrame(samples, columns=['X', 'Y', 'Z', 'cu'])
        df['loo_py_cu'] = np.nanmean(values, axis=1)
        df.to_csv('./test/testdata/output/loo_pyesi_idw_3d.csv', index=False)
    elif op=='kfold':
        values = libspatialize.kfold_esi_idw_3d(np.float32(samples[['X','Y','Z']].values), np.float32(samples[['cu']].values[:,0]), 100, 0.7, 2.0, k, np.float32(locations[['X','Y','Z']].values))
        df = pd.DataFrame(samples, columns=['X', 'Y', 'Z', 'cu'])
        df['kfold_py_cu'] = np.nanmean(values, axis=1)
...
def test_esi_kri_2d(op='estimate'):
    ...
    if op=='estimate':
        values = libspatialize.esi_kriging_2d(np.float32(samples[['x', 'y']].values), np.float32(samples[['cu']].values[:,0]), 100, 0.7, 1, 0.1, 5000.0, np.float32(locations[['X','Y']].values))
        df = pd.DataFrame(locations, columns=['X', 'Y', 'Z'])
        df['py_cu'] = np.nanmean(values, axis=1)
        df.to_csv('./test/testdata/output/pyesi_kri_2d.csv', index=False)
    elif op=='loo':
        values = libspatialize.loo_esi_kriging_2d(np.float32(samples[['x', 'y']].values), np.float32(samples[['cu']].values[:,0]), 100, 0.7, 1, 0.1, 5000.0, np.float32(locations[['X','Y']].values))
        df = pd.DataFrame(samples, columns=['X', 'Y', 'cu'])
        df['loo_py_cu'] = np.nanmean(values, axis=1)
        df.to_csv('./test/testdata/output/loo_pyesi_kri_2d.csv', index=False)
    elif op=='kfold':
        values = libspatialize.kfold_esi_kriging_2d(np.float32(samples[['x', 'y']].values), np.float32(samples[['cu']].values[:,0]), 100, 0.7, 1, 0.1, 5000.0, k, np.float32(locations[['X','Y']].values))
        df = pd.DataFrame(samples, columns=['X', 'Y', 'cu'])
        df['kfold_py_cu'] = np.nanmean(values, axis=1)
...
def test_esi_kri_3d(op='estimate'):
    ...
    if op=='estimate':
        values = libspatialize.esi_kriging_3d(np.float32(samples[['X','Y','Z']].values), np.float32(samples[['cu']].values[:,0]), 100, 0.7, 1, 0.1, 5000.0, np.float32(locations[['X','Y','Z']].values))
        df = pd.DataFrame(locations, columns=['X', 'Y', 'Z'])
        df['py_cu'] = np.nanmean(values, axis=1)
        df.to_csv('./test/testdata/output/pyesi_kri_3d.csv', index=False)
    elif op=='loo':
        values = libspatialize.loo_esi_kriging_3d(np.float32(samples[['X','Y','Z']].values), np.float32(samples[['cu']].values[:,0]), 100, 0.7, 1, 0.1, 5000.0, np.float32(locations[['X','Y','Z']].values))
        df = pd.DataFrame(samples, columns=['X', 'Y', 'Z', 'cu'])
        df['loo_py_cu'] = np.nanmean(values, axis=1)
        df.to_csv('./test/testdata/output/loo_pyesi_kri_3d.csv', index=False)
    elif op=='kfold':
        values = libspatialize.kfold_esi_kriging_3d(np.float32(samples[['X','Y','Z']].values), np.float32(samples[['cu']].values[:,0]), 100, 0.7, 1, 0.1, 5000.0, k, np.float32(locations[['X','Y','Z']].values))
        df = pd.DataFrame(samples, columns=['X', 'Y', 'Z', 'cu'])
        df['kfold_py_cu'] = np.nanmean(values, axis=1)
