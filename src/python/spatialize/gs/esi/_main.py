...
@signature_overload(pivot_arg=("base_interpolator", "idw", "base interpolator"),
                    common_args={"k": 10,
                                 "griddata": False,
                                 "n_partitions": [100],
                                 "alpha": list(np.flip(np.arange(0.97, 0.981, 0.01))),
...
def esi_hparams_search(points, values, xi, **kwargs):
    ...
    for i in it:
        ...
        args_dict = param_set.copy()
        args_dict["base_interpolator"] = kwargs["base_interpolator"]

        l_args = build_arg_list(points, values, p_xi, args_dict)
        if method == "kfold":
            l_args.insert(-1, k)
