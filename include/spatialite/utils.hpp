...
namespace sptlz{
  ...
  std::vector<T> as_1d_array(std::vector<std::vector<T>> *arr, std::vector<int> idxs){
    ...
    for(auto &record : *arr){
      for(int i=0;i<n;i++){
        result.push_back(record.at(idxs[i]));
      }
    }
  ...
  template <class T>
  std::vector<std::vector<T>> get_columns(std::vector<std::vector<T>> *arr, std::vector<int> cols){
  ...
  std::vector<int> get_folds(int n, int k, float seed){
    ...
    std::sort(permutation.begin(), permutation.end(), [](std::pair<int, float> a, std::pair<int, float> b){return(a.second<b.second);});
    for(int i=0; i<n; i++){
